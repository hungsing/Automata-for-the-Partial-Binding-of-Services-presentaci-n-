...
\end{center}
\begin{center}
\begin{tikzpicture}[->, thick, rounded corners, scale=0.8, every node/.style={transform shape}]
\node[source] (1) {};
\node[ogate] (2) [below = of 1] {};
\node[block] (3) [below left= of 2] {$p \rightarrow r:a$};
\node[block] (4) [below right= of 2] {$s \rightarrow p:b$};
\node[block] (5) [below= 2 cm of 2] {$r \rightarrow s:c$};
\draw	
	(1) edge[line] (2.north);
    \draw[line] (2.east) -| (3.north);
    \draw[line] (2.west) -| (4.north);
    \draw[line] (3.south) |- (5);
    \draw[line] (4.south) |- (5);
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Comunicación externa} a través de los canales en $\mathcal{C}$
        \item \textbf{Comunicación interna} a través de buffers
    \end{itemize} 
...
\begin{tikzpicture}[->, thick, scale=0.8, every node/.style={transform shape}]
    \node[state,initial] (q_0) {$\epsilon, \epsilon$}; 
     ...
     \node[state] (q_2) [above right = and 1.5cm of q_1 ] {$\epsilon,\epsilon$};
     \node[state] (q_8) [below right= and 1.5cm of q_1 ] {$a, b$};
     \node[state] (q_3) [right= 3.5cm of q_1 ] {$\epsilon, b$};
     \node[state] (q_4) [below=  of q_0] {$\epsilon, b$};
     \node[state] (q_5) [below = of q_4] {$\epsilon, \epsilon$};
     \node[state] (q_6) [right = 3.5cm of q_5 ] {$a, \epsilon$};
     \node[state,accepting] (q_7) [right = 3.5 of q_6 ] {$\epsilon, \epsilon$};
\draw[]        
	 ...
	 (q_8) edge[right] node{$\langle q_{111}, PR \gg a, q_{121} \rangle$} (q_3)
